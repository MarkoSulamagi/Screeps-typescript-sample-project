declare module screeps {
	...
	export interface Room {
		 ...
		 */
		find<T>(type: string, opts: {filter: any|string}): [T];
		/**
		 ...
		 */
		findPath(fromPos: RoomPosition, toPos: RoomPosition, opts: FindPathOpts) : [{x: number, y: number, dx: number, dy: number, direction: string}];
		/**
	...
	export interface RoomPosition {
		...
		findClosest<T>(type: string, opts: {filter: any|string, algorithm: string}): T;
		findClosest<T>(objects: [T|RoomPosition], opts: {filter: any|string, algorithm: string}): T;
		findClosestByRange<T>(type: string, opts: {filter: any|string }): T;
		findClosestByRange<T>(objects: [T|RoomPosition], opts: {filter: any|string }): T;
		findInRange<T>(type: string, range: number, opts: {filter: any|string, algorithm: string}): [T];
		findInRange<T>(objects: [T|RoomPosition], range: number, opts: {filter: any|string, algorithm: string}): [T];
		findPathTo(x: number, y: number, opts: FindPathOpts): [{x: number, y: number, dx: number, dy: number, direction: string}];
		findPathTo(target: RoomPosition|{pos: RoomPosition}, opts: FindPathOpts): [{x: number, y: number, dx: number, dy: number, direction: string}];
	...
	export interface Extension extends Structure {
		energy: number;
		energyCapacity: number;
		transferEnergy(target: Creep, amount: number): Result;
	}
	export interface Link extends Structure {
		cooldown: number;
		energy: number;
		energyCapacity: number;
		transferEnergy(target: Creep|Link, amount: number): Result;
	}
	export interface KeeperLair extends Structure {
		ticksToSpawn: number;
	}
	export interface Controller extends Structure {
		level: number;
		progress: number;
		progressTotal: number;
		ticksToDowngrade: number;
	}
	export interface Rampart extends Structure {
		ticksToDecay: number;
	}
	export interface Road extends Structure {
		ticksToDecay: number;
	}
	export interface Wall extends Structure {
		ticksToLive: number;
	}
	export interface Storage extends Structure {
		store: {energy: number };
	 ...
	 */
	export interface SurvivalGameInfo {
		/**
		 * Current score.
		 */
		score: number;
	...
	export interface LookAtResultMatrix {
